...
func ComputeDisks() *schema.Table {
	return &schema.Table{
		...
		Columns: []schema.Column{
			...
			{
				Name:        "sku_name",
				Description: "Name - The sku name Possible values include: 'StandardLRS', 'PremiumLRS', 'StandardSSDLRS', 'UltraSSDLRS'",
				Type:        schema.TypeString,
			...
			{
				Name:        "os_type",
				Description: "OsType - The Operating System type Possible values include: 'Windows', 'Linux'",
				Type:        schema.TypeString,
				Resolver:    schema.PathResolver("DiskProperties.OsType"),
			},
			{
				Name:        "hyperv_generation",
				Description: "HyperVGeneration - The hypervisor generation of the Virtual Machine Applicable to OS disks only Possible values include: 'V1', 'V2'",
				Type:        schema.TypeString,
				Resolver:    schema.PathResolver("DiskProperties.HyperVGeneration"),
			},
			{
				Name:        "creation_data_create_option",
				Description: "CreateOption - This enumerates the possible sources of a disk's creation Possible values include: 'Empty', 'Attach', 'FromImage', 'Import', 'Copy', 'Restore', 'Upload'",
				Type:        schema.TypeString,
				Resolver:    schema.PathResolver("DiskProperties.CreationData.CreateOption"),
			},
			{
				Name:        "creation_data_storage_account_id",
				Description: "StorageAccountID - Required if createOption is Import The Azure Resource Manager identifier of the storage account containing the blob to import as a disk",
				Type:        schema.TypeString,
			...
			},
			{
				...
				Description: "Lun - If the disk is created from an image's data disk, this is an index that indicates which of the data disks in the image to use For OS disks, this field is null",
				Type:        schema.TypeInt,
			...
			},
			{
				...
				Description: "UploadSizeBytes - If createOption is Upload, this is the size of the contents of the upload including the VHD footer This value should be between 20972032 (20 MiB + 512 bytes for the VHD footer) and 35183298347520 bytes (32 TiB + 512 bytes for the VHD footer)",
				Type:        schema.TypeBigInt,
				Resolver:    schema.PathResolver("DiskProperties.CreationData.UploadSizeBytes"),
			},
			{
				Name:        "disk_size_gb",
				Description: "DiskSizeGB - If creationDatacreateOption is Empty, this field is mandatory and it indicates the size of the disk to create If this field is present for updates or creation with other options, it indicates a resize Resizes are only allowed if the disk is not attached to a running VM, and can only increase the disk's size",
				Type:        schema.TypeInt,
			...
			},
			{
				...
				Description: "Enabled - Set this flag to true and provide DiskEncryptionKey and optional KeyEncryptionKey to enable encryption Set this flag to false and remove DiskEncryptionKey and KeyEncryptionKey to disable encryption If EncryptionSettings is null in the request object, the existing settings remain unchanged",
			...
			},
			{
				...
				Description: "EncryptionSettingsVersion - Describes what type of encryption is used for the disks Once this field is set, it cannot be overwritten '10' corresponds to Azure Disk Encryption with AAD app'11' corresponds to Azure Disk Encryption",
				Type:        schema.TypeString,
			...
			},
			{
				...
				Description: "DiskIOPSReadOnly - The total number of IOPS that will be allowed across all VMs mounting the shared disk as ReadOnly One operation can transfer between 4k and 256k bytes",
				Type:        schema.TypeBigInt,
				Resolver:    schema.PathResolver("DiskProperties.DiskIOPSReadOnly"),
			},
			{
				Name:        "disk_mbps_read_only",
				Description: "DiskMBpsReadOnly - The total throughput (MBps) that will be allowed across all VMs mounting the shared disk as ReadOnly MBps means millions of bytes per second - MB here uses the ISO notation, of powers of 10",
				Type:        schema.TypeBigInt,
			...
			},
			{
				...
				Description: "Type - Possible values include: 'EncryptionAtRestWithPlatformKey', 'EncryptionAtRestWithCustomerKey', 'EncryptionAtRestWithPlatformAndCustomerKeys'",
				Type:        schema.TypeString,
				Resolver:    schema.PathResolver("DiskProperties.Encryption.Type"),
			},
			{
				Name:        "max_shares",
				Description: "MaxShares - The maximum number of VMs that can attach to the disk at the same time Value greater than one indicates a disk that can be mounted on multiple VMs at the same time",
				Type:        schema.TypeInt,
			...
			},
			{
				...
				Description: "NetworkAccessPolicy - Possible values include: 'AllowAll', 'AllowPrivate', 'DenyAll'",
				Type:        schema.TypeString,
