...
func buildNetworkVirtualNetworksMock(t *testing.T, ctrl *gomock.Controller) services.Services {
	...
	require.Nil(t, faker.FakeDataSkipFields(&(*vn.Subnets)[0].IPConfigurations), []string{"PrivateEndpointProperties"})
	require.Nil(t, faker.FakeDataSkipFields(&(*vn.Subnets)[0].PrivateEndpoints), []string{"Subnet"})
